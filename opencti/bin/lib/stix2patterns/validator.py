...
def run_validator(pattern, stix_version=DEFAULT_VERSION):
    ...
    if not brackets_check(pattern_str):
        err_messages.insert(
            0, "FAIL: Error found at line 1:0. input is missing square brackets"
        )
...
def validate(
    user_input, stix_version=DEFAULT_VERSION, ret_errs=False, print_errs=False
):
    """
...
def main():
    ...
    if args.file:
        ...
        nextpattern = functools.partial(
            six.moves.input, "Enter a pattern to validate: "
        )
