...
class MetricWrapperBase(object):
    ...
    def __repr__(self):
        ...
        return "{0}.{1}({2})".format(
            metric_type.__module__, metric_type.__name__, self._name
        )
    ...
    def labels(self, *labelvalues, **labelkwargs):
        ...
        if self._labelvalues:
            raise ValueError(
                "%s already has labels set (%s); can not chain calls to .labels()"
...
class Info(MetricWrapperBase):
    ...
    def info(self, val):
        ...
        if self._labelname_set.intersection(val.keys()):
            raise ValueError(
                "Overlapping labels for Info metric, metric: %s child: %s"
